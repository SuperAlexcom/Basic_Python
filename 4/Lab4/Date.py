class Date:
    ...
    def __init__(self, day = None, month = None, year = None):
        ...
        if OK and\
            ...
            0 < month < 13 and\
            year > 0 and\
            (month in self.bigMonths and day < 32 or\
             month not in self.bigMonths and month != 2 and day < 31 or\
                    month == 2 and self.IsYearBisextile(year) and day < 30 or\
                    month == 2 and not self.IsYearBisextile(year) and day < 29):
            self.__day = day
            self.__month = month
            self.__year = year
    ...
    def day(self, newDay):
        maxDay = 0
        if self.__month in self.bigMonths:
            maxDay = 31
        elif self.month != 2:
            maxDay = 30
        elif self.IsYearBisextile(self.year):
            maxDay = 29
        else:
            maxDay = 28
